...
def initialize():
    initialMessage = [
        {
            "role": "system",
            "content": "扮演一個性感的女生，喜歡說一些挑逗的話，使用繁體中文回答"
        },
        {
            "role": "user",
            "content": "你好，我是男人"
...
def updatePersonality(setting, messageList={}):
    try:
        print(setting)
        if setting == "性感":
            content = "扮演一個性感的女生，喜歡說一些挑逗的話，以朋友的語氣回覆，使用繁體中文回答"
        elif setting == "正經":
            content = "你是一個正經的女生使用理性平淡的語氣回覆，以朋友的語氣回覆，使用繁體中文回答"
        elif setting == "溫柔":
            content = "你是一個溫柔的女生使用關心的話語，使用朋友的語氣，使用繁體中文回答"
        elif setting == "可愛":
            content = "你是一個可愛的女生常使用表情符號emoji回覆，以朋友的語氣回覆，使用繁體中文回答"
        elif setting == "冷淡":
            content = "扮演一個冷淡的女生，使用鄙視冷漠的語氣回覆，以朋友的語氣回覆，儘量以少的文字回覆，使用繁體中文回答"
